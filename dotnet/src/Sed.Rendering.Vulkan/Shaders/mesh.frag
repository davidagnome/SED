...
void main()
{
    if (pc.mode == 1u) {                 // flat: markers / selection
    ...
    if (pc.skyMode == 1u) {              // ceiling sky: project to ceiling plane, map world XY
        ...
        light = 1.0;
    } else if (pc.skyMode == 2u) {      // horizon sky: cylindrical, scrolls with view
        vec3 dir = normalize(vWorld - pc.camPos.xyz);
        uv = vec2(atan(dir.x, dir.y) / (2.0 * PI), 0.5 - asin(clamp(dir.z, -1.0, 1.0)) / PI);
        light = 1.0;
    }
